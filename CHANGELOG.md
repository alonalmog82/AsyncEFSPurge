...
- **`--phase3-batch-size N` iterative mode (env: `EFSPURGE_PHASE3_BATCH_SIZE`)** — When `--phase3-only` is set together with a positive batch size, the sidecar is streamed in batches of `N` unique paths and each batch is drained by the existing Phase 3 cascade-deletion pass.  Peak memory is bounded to roughly `N × ~500 bytes/Path` instead of the full sidecar, which is critical when the sidecar has grown to millions of entries.  Duplicate entries across batches are safe (a repeat delete on an already-removed dir is a no-op).  `0` (default) keeps the historical load-all behaviour.  Recommended: `100000` for sidecars in the 1M–10M range.
- **`--phase3-deletion-workers N` (env: `EFSPURGE_PHASE3_DELETION_WORKERS`)** — Number of concurrent worker coroutines during Phase 3 empty-directory deletion (both the first pass and the cascading iterations).  Default `0` keeps the historical behaviour of reusing `--max-concurrent-discovery` (default 20), which is a safe cap against the scandir-executor-flood stall documented in 2.0.4.  Operators cleaning up very large accumulated empty-dir backlogs (multi-million dirs from a whale first-pass) can raise this to 50–200 to speed up Phase 3 throughput.  **When raising above ~50, also raise `--scandir-executor-threads` in proportion** (rule of thumb: at least 2× this value) — otherwise the workers queue on the scandir executor and stall.  The `deletion_semaphore` still caps concurrent `rmdir` I/O independently of worker count.

### Fixed
- **`--phase3-only` was removing the empty-dirs sidecar even when the deletion pass aborted early** (memory-critical circuit breaker fires before / during the deletion pass).  In that scenario the operator would see a "successful" job exit with zero actual work done AND the accumulated candidates gone from the sidecar.  The sidecar is now removed only when the drain actually completes (`self._checkpoint_requested` is False at the end of the phase-3-only path).  A warning is logged when the sidecar is preserved for retry.
